...
(function() {
    ...
    function DomAdapter(){
        ...
        this.addLootSummary = function(){
            var returning = jQuery("table.troop_details.inReturn");
        ...
        this.addMerchantSummary = function(){
            var merchants = jQuery("table.traders");
            if (merchants.length == 0) {
                return;
            }

            // Find the index of the last arriving mechant
            var endIndex = Number.MAX_VALUE;
            jQuery("#merchantsOnTheWay h4").each(function() {
                const header = jQuery(this);

                if (header.text().trim().startsWith("Own")) {
                    endIndex = header.index() - 1;
                }
            });
        ...
        function ResourceChart() {
            this.total = [0, 0, 0, 0];
            this.production = hb.activeVillage.production.numbers;

            this.entries = [[], [], [], []];
            // Initial resource amounts
            ["#l1", "#l2", "#l3", "#l4"].map(id => toNumbersOnly(jQuery(id).text()))
                    .forEach((val, index) => this.entries[index].push({x: new Date(), y: val}));

            this.addEntry = function(resource, date, loot) {
                this.total[resource] += loot;

                var previous = this.entries[resource].last();

                if (previous.x.getTime() == date.getTime() + 1) {
                    // Two returning on same second => merge values
                    previous.y += loot;
                } else {
                    // Account for production
                    var hourDiff = (date.getTime() - previous.x.getTime()) / 3600000;
                    var yBeforeLoot = Math.round(previous.y + hourDiff * this.production[resource]);
            ...
            this.render = function() {
                new Chartist.Line('.ct-chart',
                    {
                        series: this.entries.map(entry => ({data: entry}))
                    },
                    {
                        showPoint: false,
        ...
        this.addTroopProductionSummary = function(){
            if (jQuery(".village3 #overview").length == 0)
                return;

            var queue = {};

            jQuery(".village3 #overview td.tro img").each(function(){
                console.log(jQuery(this).attr('class'));
                var classes = jQuery(this).attr('class');
                if(queue[classes] == null){
                    queue[classes] = 0;
                }
                queue[classes] += toNumbersOnly(jQuery(this).attr("alt"));
            });
            ...
            for(var type in queue){
                dom += '<img class="'+type+'" src="img/x.gif"> x '+ queue[type] +'<br>';
            }

            jQuery(".village3").append("<h3>Totalt i kö:</h3> " + dom);
            ...
            if(attackDetails !== null){
                ...
                for(var m in movements){
                    ...
                    if(movement.type == "ATTACK" || movement.type == "ATTACK_SELF" || movement.type == "ATTACK_OUT"){
                        if(movement.url == null){

                            var attackDetails = {"type": "ATTACK", "from": "???", "time": Date.now()+60*100};
                            this.persistentData.activities.push(attackDetails);
                            this.saveData();
                            console.log("alerting of incoming attack but no thingamabob in village");

                            this.plebbeAlerter.sendAlert(this.user, data.type + " kommer i " + (this.activeVillage.name || "???") + " från " + data.from + " kl ??", Date.now()/1000);
...
GM_setValue('harryBoyMP', {
            ...
            */
            ...
            this.domAdapter.addMerchantSummary();
            this.domAdapter.addTroopProductionSummary();
