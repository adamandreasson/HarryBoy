...
(function() {
    ...
    function DomAdapter(){
        ...
        this.addLootSummary = function(){
            ...
            getResources().forEach((val, index) => lootEntries[index].push({x: new Date(), y: val}));

            var production = hb.activeVillage.production.numbers;

            // Each returning raid
            returning.each(function(){
                var at = jQuery(this).find(".at").text().trim().substr(3);
                at = moment(at, "HH:mm:ss").toDate();
            ...
            function getResources() {
                return ["#l1", "#l2", "#l3", "#l4"].map(id => toNumbersOnly(jQuery(id).text()));
            }
            ...
            function initChart() {
                ...
                var dom = totalLoot
                    ...
                    .join("");
                dom = '<h3 style="text-align:center">Incoming loot: ' + dom + '</h3>';
            ...
            function renderChart() {
                new Chartist.Line('.ct-chart',
                    {
                        series: lootEntries.map(entry => ({data: entry}))//.slice(0, 3)
                    },
                    {
                        showPoint: false,
                        axisX: {
                            ...
                            divisor: 5,
                            labelInterpolationFnc: val => moment(val).format('HH:mm:ss')
                        }
                    });
            }
        };
